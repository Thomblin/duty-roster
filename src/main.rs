...
mod tests {
    ...
    fn test_person_state_formatting() {
        ...
        assert!(people[0].weekday_counts().contains_key(&chrono::Weekday::Mon));
        assert!(people[0].weekday_counts().contains_key(&chrono::Weekday::Wed));
        ...
        assert!(people[1].weekday_counts().contains_key(&chrono::Weekday::Tue));
    ...
    fn test_args_parsing() {
        ...
        let args = Args::parse_from(&["duty-roster", "--config", "custom.toml", "--out", "output.csv", "--cli"]);
    ...
    fn test_store_csv_invalid_path() {
        ...
        let invalid_path = PathBuf::from("/invalid/path/that/should/not/exist")
            .join(format!("file_{}.csv", std::time::SystemTime::now().elapsed().unwrap().as_nanos()));
