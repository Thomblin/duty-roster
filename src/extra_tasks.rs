...
pub fn apply_extra_tasks(assignments: &mut Vec<Assignment>, config: &Config) {
    ...
    for task in extra_tasks {
        ...
        for &i in &all_slots {
            *total_appearances.entry(assignments[i].base_person.clone()).or_default() += 1;
        }
        ...
        for &i in &all_slots {
            slots_by_date.entry(assignments[i].date).or_default().push(i);
        }
        ...
        loop {
            ...
            if max_count - min_count <= 1 { break; }
            ...
            for (&date, slots) in &slots_by_date {
                for &i in slots {
                    person_dates.entry(assignments[i].base_person.clone()).or_default().push(date);
                }
            }
            ...
            for dates in person_dates.values_mut() { dates.sort(); }
            ...
            let mut min_people: Vec<String> = counts.iter()
                .filter(|&(_, &c)| c == min_count).map(|(n, _)| n.clone()).collect();
            min_people.sort();
            ...
            'bfs: for start in &min_people {
                ...
                let mut queue: VecDeque<(String, Vec<(chrono::NaiveDate, String)>)> = VecDeque::new();
            ...
            let Some(chain) = path else { break; };
            ...
            // etc.
            let start_person = min_people.iter()
                .find(|p| {
                    let first_date = chain[0].0;
                    person_dates.get(*p).map(|ds| ds.contains(&first_date)).unwrap_or(false)
            ...
            for (date, take_from) in &chain {
                ...
                let give_idx = slots_by_date[date].iter().copied()
                    .find(|&i| assignments[i].base_person == give_to).unwrap();
                // Find slot index for take_from on this date
                let take_idx = slots_by_date[date].iter().copied()
                    .find(|&i| assignments[i].base_person == *take_from).unwrap();
                // Transfer
...
mod tests {
        ...
        Config {
            ...
            group: vec![
                ...
                Group {
                    name: "Doe".to_string(),
                    place: "Doe".to_string(),
                    members: vec![Member { name: "Charlie".to_string() }],
                },
            ],
            rules: Rules {
                sort: vec![],
    ...
    fn extra_task_distributed_evenly_across_dates() {
        ...
        let dates = [date(2025, 9, 1), date(2025, 9, 2), date(2025, 9, 3), date(2025, 9, 4)];
        ...
        let alice_count = assignments.iter().filter(|a| a.person == "Alice Maier 🪴").count();
        let bob_count = assignments.iter().filter(|a| a.person == "Bob Maier 🪴").count();

        // Should be 2 each (4 dates, 2 eligible people)
        assert_eq!(alice_count + bob_count, 4);
        assert!((alice_count as i32 - bob_count as i32).abs() <= 1, "Distribution uneven: alice={alice_count} bob={bob_count}");
    }
    ...
    fn two_tasks_same_eligible_both_assigned_per_date() {
        ...
        let config = make_config(vec![
            ExtraTask { name: "🪴".to_string(), groups: vec!["Maier".to_string()] },
            ExtraTask { name: "🪟".to_string(), groups: vec!["Maier".to_string()] },
        ]);
        ...
        let count = |name: &str, icon: &str| assignments.iter()
            .filter(|a| a.person.contains(icon) && a.base_person == name).count();
    ...
    fn max_gap_is_one_across_eligible_pool() {
        ...
        let count = |name: &str| -> usize {
            assignments.iter().filter(|a| a.person.contains("🪟") && a.base_person == name).count()
        };
        ...
        assert!((a as i32 - c as i32).abs() <= 1, "gap too large: Alice={a} Charlie={c}");
    }
    ...
    fn two_tasks_different_eligibility_stay_balanced() {
        ...
        let count = |name: &str, icon: &str| -> usize {
            assignments.iter().filter(|a| a.person.contains(icon) && a.base_person == name).count()
        };

        let alice_w = count("Alice Maier", "🪟");
        let bob_w = count("Bob Maier", "🪟");
        assert_eq!(alice_w + bob_w, 10, "all 10 dates need 🪟");
        assert!((alice_w as i32 - bob_w as i32).abs() <= 1,
    ...
    fn large_scale_gap_is_at_most_one() {
        ...
        let sonn_names: Vec<&str> = vec!["Ana", "Antonio", "Rita", "Jochen", "Julia", "Marcel", "Karolin", "Ramin"];
        let stern_names: Vec<&str> = vec!["Corinna", "Stephan", "Henning", "Nina", "XX", "XY", "Charlotte", "Christina"];
        ...
        let config = Config {
            ...
            group: {
                ...
                for name in &sonn_names {
                    groups.push(Group {
                        ...
                        members: vec![Member { name: name.to_string() }],
                    });
                }
            ...
            rules: Rules { sort: vec![], filter: vec![] },
            extra_task: Some(vec![
                ExtraTask { name: "🪴".to_string(), groups: sonn_names.iter().chain(stern_names.iter()).map(|s| s.to_string()).collect() },
                ExtraTask { name: "🪟".to_string(), groups: sonn_names.iter().chain(stern_names.iter()).map(|s| s.to_string()).collect() },
            ]),
        };
        ...
        for icon in ["🪴", "🪟"] {
            let counts: Vec<usize> = sonn_names.iter().chain(stern_names.iter()).map(|name| {
                let full = format!("{name} {name}");
                assignments.iter().filter(|a| a.person.contains(icon) && a.base_person == full).count()
            }).collect();
            let max = *counts.iter().max().unwrap();
            let min = *counts.iter().min().unwrap();
            assert!(max - min <= 1, "{icon} gap too large: max={max} min={min} counts={counts:?}");
        }
    }
    ...
    fn person_with_fewer_appearances_gets_proportional_quota() {
        ...
        let alice = assignments.iter().filter(|a| a.base_person == "Alice Maier" && a.person.contains("🪟")).count();
        let bob = assignments.iter().filter(|a| a.base_person == "Bob Maier" && a.person.contains("🪟")).count();
        assert_eq!(alice + bob, 2, "2 dates must be covered");
        // With flat quotas: Alice quota=2, Bob quota=1 — gap ≤ 1
        assert!((alice as i32 - bob as i32).abs() <= 1, "gap too large: Alice={alice} Bob={bob}");
    }
